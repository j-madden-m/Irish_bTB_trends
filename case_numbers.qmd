...
# this comes from "gif_lesion_prep"
# no longer come from there, comes directly from year_2025
# we need to run this twice - once without filtering (for policy plot)
# and one with filtering by 2007 for the document itself
# all_cases_collapsed <- readRDS(paste0("data/edited_output/all_cases_collapsed.rds")) %>%     filter(year > 2007) 
...
min_year <- min((all_cases_collapsed %>% 
                  filter_active_year())$year)
max_year <- max((all_cases_collapsed %>% 
                  filter_active_year())$year)
years_for_study <- c(min_year:max_year)


# extract min/max years when incorporating month (i.e. active data)
min_year_active <- min(all_cases_collapsed$year)
max_year_active <- max(all_cases_collapsed$year)
...
case_count_year_05 <- read_csv("data/edited_output/case_count_year_05.csv")
case_count_year <- case_count_function(all_cases_collapsed %>% 
                                         filter_active_year(), "year")
#case_count_year
...
case_count_plot_fun(case_count_year, "year", "cases", max_year_var = max_year, month_interactive = TRUE)


# adding 2005-2007 data for policy plot
case_count_year_05_to_current <- case_count_year_05 %>%
  #mutate(herd_prevalence = as.character(herd_prevalence)) %>%
  filter(year %in% c(2005:2007)) %>%
  bind_rows(case_count_year)
# saving data
write_csv(case_count_year_05_to_current, "data/edited_output/case_count_year_05_to_current.csv")

# *************************************************************************
# save data for the paper
saveRDS(case_count_year, paste0("data/edited_output/for_paper/case_count_year_for_paper.rds"))
# *************************************************************************
```
...
case_count_plot_fun(case_count_month, "year_month", "cases", max_year_var = max_year_active, month_interactive = TRUE)
```

### by herd-type

```{r}
#| label: fig-caseovertimeherdtype
#| include: TRUE
#| echo: FALSE
#| eval: TRUE
#| warning: FALSE
...
#| fig.width: 10
#| fig.height: 8 
#| fig.cap: "Number of bTB cases by year, herd-type and diagnostic method."
tmp <- case_count_function(all_cases_collapsed %>% 
                                         filter_active_year(), grouping_vars = c("year", "herd_type_ml_description"))
ggplot(tmp %>% 
         filter(!herd_type_ml_description  %in% c(NA, "Unknown")), aes(x = year, y = cases, color = best_estimate_gif_skin_lab_string)) +
  geom_line(linewidth = 1.5, alpha = 1) +
  # labs(x = "Year",
  #      y = "Lesion prevalence",
  #      title = title_lesions_reactors_ahcs,
  #      caption = paste0(all_lesions_vector, "\n",
  #                       reactor_lesion_definition)) +
  theme_jamie +
  scale_x_continuous(breaks = nth_element(c(unique(tmp$year)), 1, 1)) +
  scale_y_continuous(labels = scales::label_comma()) +
  scale_colour_nejm(name = "Detection type") +
  facet_wrap(vars(herd_type_ml_description), dir = "h", ncol = 2) +
  theme_45_x 
```
...
proportion_cases_function(all_cases_collapsed %>% 
                                         filter_active_year(), herd_type, interactive = TRUE)
...
#proportion_cases_function(all_cases_collapsed, herd_type)
proportion_cases_function(all_cases_collapsed %>% 
                                         filter_active_year(), herd_type_ml_description, interactive = TRUE)
...
#proportion_cases_function(all_cases_collapsed, herd_type)
proportion_cases_function(all_cases_collapsed %>% 
                                         filter_active_year(), herd_type_ml_sub_class_description, interactive = TRUE)
...
# adding 2005-2007 data for policy plot
prev_df_05 <- read_csv("data/edited_output/herd_level_prevalance_data_05.csv")
prev_df_05_to_current <- prev_df_05 %>% 
  mutate(herd_prevalence = as.character(herd_prevalence)) %>% 
  filter(year %in% c(2005:2007)) %>% 
  bind_rows(prev_df)
# saving data
write_csv(prev_df_05_to_current, "data/edited_output/prev_df_05_to_current.csv")
...
prev_master_tb_brock <- collap(master_tb %>% 
                                 filter_active_year(), ~ herd_no + skin_fixed_test_date_year, custom = list(collap_fun_greater_1 = c("all_cases"),
                                                                                              ffirst = c("herd_type_ml_description", "herd_type_ml_sub_class_description")))
...
# *************************************************************************
# save data for the paper
saveRDS(prev_brock_df, paste0("data/edited_output/for_paper/prev_brock_df_for_paper.rds"))
# *************************************************************************
