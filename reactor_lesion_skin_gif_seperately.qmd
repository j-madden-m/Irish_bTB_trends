...
# *************************************************************************
# create function to filter data according to whether plot is yearly or
# monthly, in case of yearly, we want to ensure, we have the full data for that
# year
# prob can move this to kilkenny functions.r
filter_active_year <- function(data) {
  
  if ("year" %in% names(data)) {
    data %>% 
      dplyr::filter(year != 2026)
    
  } else if ("skin_fixed_test_date_year" %in% names(data)) {
    data %>% 
      dplyr::filter(skin_fixed_test_date_year != 2026)
    
  } else {
    stop("Unknown data structure: expected `year` or `skin_fixed_test_date_year` column.")
  }
}
# filter_active_year <- function(data) {
#   data %>% 
#     # NB you need dplyr:: here
...
#| fig.height: 8 
#| fig.cap: !expr paste0("Overall lesion prevalence within SICTT and ", gif_string, " positive animals at slaughter.")
lesion_rate_by_gif_or_skin_year <- lesion_rate_by_gif_or_skin(all_cases_collapsed %>% 
                                                                filter_active_year(), c(year))
...
lesion_rate_by_gif_or_skin_year_ahcs <- lesion_rate_by_gif_or_skin(all_cases_collapsed %>% filter_active_year(), c(year, herd_type))
...
lesion_rate_by_gif_or_skin_year_brock <- lesion_rate_by_gif_or_skin(all_cases_collapsed %>% filter_active_year(), c(year, herd_type_ml_description))
...
permit_lesion_head_chest_type_by_gif_skin_year <- permit_lesion_head_chest_type_by_gif_skin(all_cases_collapsed %>%                                                                     filter_active_year(), c(year))
...
permit_lesion_head_chest_type_by_gif_skin_year_ahcs <- permit_lesion_head_chest_type_by_gif_skin(all_cases_collapsed %>% filter_active_year(), c(year, herd_type))
...
permit_lesion_head_chest_type_by_gif_skin_year_brock <- permit_lesion_head_chest_type_by_gif_skin(all_cases_collapsed %>%                                                                             filter_active_year(), c(year, herd_type_ml_description)) 
...
-   Investigate `r gif_string` avain readings pattern.
