...
This document presents the prevalence of animals with confirmed *m.bovis* lesions found at routine slaughter (also known as ‘factory lesion cases’). 

**NOTE: This is a draft, live document - it will be updated periodically as we obtain more data. The most current date may vary between output as we do not receive all the relevant data at once e.g., we obtain data for the previous year from the AIM system in the later half of the current year and as a result we won't be able to estimate the lesion rate within healthly slaughtered animals. This also means that we don't have last years herd-type or herd-size AIM related metrics. In the case of herd-type, we carry the latest estimate forward until the new data comes in.**
