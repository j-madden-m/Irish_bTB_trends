...
healthy_lesion_rate <- healthy_lesion_rate %>% 
   mutate(herd_type_ml_description = fct_relevel_herd(herd_type_ml_description),
          herd_type = fct_relevel_herd_ahcs(herd_type))
...
healthy_lesion_rate_month <- healthy_lesion_rate_month %>% 
   mutate(herd_type_ml_description = fct_relevel_herd(herd_type_ml_description),
          herd_type = fct_relevel_herd_ahcs(herd_type))
...
This page presents the prevalence of animals with confirmed *m.bovis* lesions found at routine slaughter (also known as ‘factory lesion cases’).

**NOTE: This section relies heavily on data from AIM and there is usually quite a lag on when this data is received.**
...
no_animals_sent_to_slaughter_month <- healthy_lesion_rate_month %>% 
  ...
  mutate(year = year(year_month),
         ...
         slaughter_month = lubridate::month(year_month, label = TRUE))
...
To explore seasonality further, we plotted the same plot in a different format:

```{r}
#| label: fig-nosenttoslaughtermonthseasonal
#| include: TRUE
#| echo: FALSE
#| eval: TRUE
#| warning: FALSE
...
ggplot(no_animals_sent_to_slaughter_month, aes(x = slaughter_month, y = no_of_animals_sent_to_slaughter)) +
  ...
  labs(
    x = "Month",
    y = "Number of animals sent to slaughter",
    color = "Year",
    title = "Number of animals sent to slaughter by year"
  ) +
  theme_jamie +
  theme_45_x +
...
ggplot(no_animals_sent_to_slaughter_ahcs_herd_type, aes(x = year, y = no_of_animals_sent_to_slaughter, color = herd_type)) +
  ...
  scale_color_manual(values = ahcs_cols, name = "Herd-type") +
  scale_y_continuous(labels = scales::comma) +
...
ggplot(no_animals_sent_to_slaughter_brock_herd_type, aes(x = year, y = no_of_animals_sent_to_slaughter, color = herd_type_ml_description)) +
  ...
  theme_jamie +
  #scale_colour_nejm(name = "Herd-type") +
  scale_color_manual(values = brock_cols, name = "Herd-type") +
  scale_y_continuous(labels = scales::comma) +
...
```{r}
#| label: fig-allhealthylesionprevmonthlyseasonal
#| include: TRUE
#| echo: FALSE
#| eval: TRUE
#| warning: FALSE
...
#| fig.width: 12
#| fig.height: 8 
#| fig.cap: "Overall lesion prevalence at routine slaughter by month and years."
ggplot(healthy_lesion_rate_all_month, aes(x = slaughter_month, y = lesion_rate_overall)) +
  geom_line(aes(color = year, group = year), size = 1) +
  scale_color_viridis_c(
    option = "plasma",  # You can try "viridis", "inferno", "magma", etc.
    name = "Year"
  ...
  labs(
    x = "Month",
    y = "Lesion prevalence in healthy slaughter animals",
    color = "Year",
    title = "Lesion prevalence in healthy slaughter animals"
  ) +
  scale_y_continuous(labels = scales::label_percent(scale = 100)) +
  theme_jamie +
  theme_45_x +
...
And a simple count of lesions in healthy slaughter animals:

```{r}
#| label: fig-allhealthylesioncountmonthlyseasonal
#| include: TRUE
#| echo: FALSE
#| eval: TRUE
#| warning: FALSE
...
#| fig.width: 12
#| fig.height: 8 
#| fig.cap: "Overall lesion count at routine slaughter by month and years."
ggplot(healthy_lesion_rate_all_month, aes(x = slaughter_month, y = no_of_lab_only)) +
  geom_line(aes(color = year, group = year), size = 1) +
  scale_color_viridis_c(
    option = "plasma",  # You can try "viridis", "inferno", "magma", etc.
    name = "Year"
  ...
  labs(
    x = "Month",
    y = "Count of lesions in healthy slaughter animals",
    color = "Year",
    title = "Count of lesions in healthy slaughter animals"
  ) +
  # scale_y_continuous(labels = scales::label_percent(scale = 100)) +
  theme_jamie +
  theme_45_x +
...
saveRDS(healthy_lesion_rate_brock_herd_type, paste0("data/edited_output/for_paper/healthy_lesion_rate_brock_herd_type.rds"))
# *************************************************************************
...
From DAFM document <https://teagasc.ie/wp-content/uploads/2025/05/How-is-bovine-TB-detected-at-slaughter.pdf>:
...
-   Is it worth considering additionally testing for herds with a lot of chest detections (round of `r gif_string` testing)? Or even herds with animals with multiple lesions (more advanced infection)? Investigate this further.
...
-   Does it suggests that there a cohort of riskier herds, that send their animals to slaughter later in the year (and perhaps, do not send animals earlier in the year and so we miss their infections)- Investigate this further.
...
-   It would be really interesting to get the age profile of these – to do. What we are seeing could be a certain aged animal that tends to get sent to slaughter later in the year.
...
-   Update 05/02/2026 - from about 2027 onwards, "multiple lesions" will no longer be an option, the location of the lesions will be recorded. Additionally, the recording will be done in the factory (as opposed to recording from permit notes at DVO).
...
- Gomez-Buendia A, Triguero-Ocaña R, Pozo P, Muñoz-Mendoza M, Alvarez-Fidalgo M, Mourelo J, Romero B, Alvarez J. Abattoir surveillance: Identifying risk factors associated with bovine tuberculosis lesion detection in a low prevalence region. Prev Vet Med. 2026 Jan;246:106710. doi: <https://www.sciencedirect.com/science/article/pii/S0167587725002958> Epub 2025 Oct 6. PMID: 41076935.
...
- Gavier-Widén D, Cooke MM, Gallagher J, Chambers MA, Gortázar C. A review of infection of wildlife hosts with Mycobacterium bovis and the diagnostic difficulties of the 'no visible lesion' presentation. N Z Vet J. 2009 Jun;57(3):122-31. <https://pubmed.ncbi.nlm.nih.gov/19521460/> PMID: 19521460.


- Byrne AW, Graham J, Brown C, Donaghy A, Guelbenzu-Gonzalo M, McNair J, Skuce R, Allen A, McDowell S. Bovine tuberculosis visible lesions in cattle culled during herd breakdowns: the effects of individual characteristics, trade movement and co-infection. BMC Vet Res. 2017 Dec 29;13(1):400. doi: 10.1186/s12917-017-1321-z. PMID: 29284483; PMCID: PMC5747088. <https://pmc.ncbi.nlm.nih.gov/articles/PMC5747088/>

- Byrne AW, Graham J, Brown C, Donaghy A, Guelbenzu-Gonzalo M, McNair J, Skuce RA, Allen A, McDowell SW. Modelling the variation in skin-test tuberculin reactions, post-mortem lesion counts and case pathology in tuberculosis-exposed cattle: Effects of animal characteristics, histories and co-infection. Transbound Emerg Dis. 2018 Jun;65(3):844-858. doi: 10.1111/tbed.12814. Epub 2018 Jan 23. PMID: 29363285. <https://pubmed.ncbi.nlm.nih.gov/29363285/>

- Collins, John D., and John Hannan. “Tuberculosis in Cattle: Lesions Seen at Slaughter.” 
University College Dublin. Centre for Veterinary Epidemiology and Risk Analysis, July 1994. <https://researchrepository.ucd.ie/server/api/core/bitstreams/6b03a180-e0a1-4292-911a-b7a01033f45e/content>

- Drehoff, C. C., Munden, K. C., Ray, C., Martinez, H., Robbe-Austerman, S., & Lombard, J. E. (2026). An Examination of Gross Lesions Associated with Bovine Tuberculosis in the U.S. Pathogens, 15(4), 350. <https://doi.org/10.3390/pathogens15040350>
...
Robinson SE, Christley RM. Identifying temporal variation in reported births, deaths and movements of cattle in Britain. BMC Vet Res. 2006 Mar 30;2:11. <https://pubmed.ncbi.nlm.nih.gov/16573832/> PMID: 16573832; PMCID: PMC1440854.
