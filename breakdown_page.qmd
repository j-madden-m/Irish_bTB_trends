...
source("R_files/bd_count_by_month_fun.R")
...
bd_df <- readRDS(bd_df_file) 
# for the majority of this document, we want it by bd_end_yr
# but at start we want bd_df_start_yr so i have just created separate file for that
bd_df_start_df <- bd_df %>% 
  filter(bd_start_yr > 2007 & bd_start_yr < 2026)

bd_df <- bd_df %>% 
  filter(bd_end_yr > 2007 & bd_end_yr < 2026) 
bd_df_labels <- as.list(sjlabelled::get_label(bd_df))
bd_df <- sjlabelled::remove_all_labels(bd_df)
# drop years before 2008
bd_df <- bd_df %>% 
  filter(bd_end_yr > 2007) %>% 
  mutate(bd_start_yr_f = as.character(bd_start_yr),
         bd_end_yr_f = as.character(bd_end_yr)) %>% 
...
min_year <- min(bd_df$bd_end_yr)
max_year <- max(bd_df$bd_end_yr)
years_for_study <- c(min_year:max_year)


# bd_df_start_df
min_year_start <- min(bd_df_start_df$bd_start_yr)
max_year_start <- max(bd_df_start_df$bd_start_yr)
years_for_study_start <- c(min_year_start:max_year_start)
```
...
#| fig.width: 12
#| fig.height: 8 
#| fig.cap: "Number of BDs per month."
# overall
bd_df_per_month <- bd_df_start_df %>% 
  mutate(year_month = floor_date(bd_start , "month")) %>% 
  #select(herd_no, bd_start, year_month) 
  group_by(year_month) %>% 
  summarise(no_of_breakdowns_starting_in_month = n()) %>% 
  ungroup() %>% 
  mutate(year = year(year_month))

min_max_dates_df <- min_max_geom_react_with_var(bd_df_per_month, no_of_breakdowns_starting_in_month)
bd_count_month_plot_fun(bd_df_per_month, "year_month", "no_of_breakdowns_starting_in_month", month_interactive = FALSE)
```
...
### Per month and by herd-type

```{r}
#| label: fig-bdpermonthherdtype
#| include: TRUE
#| echo: FALSE
#| eval: TRUE
#| warning: FALSE
...
#| fig.width: 12
#| fig.height: 8 
#| fig.cap: "Number of BDs per month and herd-type."
# by herd type
bd_df_per_month_herd_type <- bd_df_start_df %>% 
  mutate(year_month = floor_date(bd_start , "month")) %>% 
  #select(herd_no, bd_start, year_month) 
  ...
  ungroup() %>% 
  mutate(year = year(year_month),
         hover_text_point_in_df = paste0("Date: ", year_month,
                                         "\n",
                                         "Herd-type: ", herd_type_ml_description,
                                         "\n",
                                         "No. of BDs starting in month: ", no_of_breakdowns_starting_in_month),
         hover_text_line = paste0("Herd-type: ", herd_type_ml_description))

min_max_dates_df <- min_max_geom_react_with_var(bd_df_per_month_herd_type, no_of_breakdowns_starting_in_month)
#bd_count_plot_fun(tmp, "year_month", "no_of_breakdowns_starting_in_month", color_var = "herd_type_ml_description", month_interactive = FALSE) 
bd_count_month_plot_fun(bd_df_per_month_herd_type, "year_month", "no_of_breakdowns_starting_in_month", color_var = "herd_type_ml_description", month_interactive = TRUE) 
```
...
#| fig.width: 12
#| fig.height: 8 
#| fig.cap: "Proportion of BDs ending per year by herd-type."
prop_herd_type_bds <- bd_df %>% 
  #filter(bd_first_skin_test_type == 8) %>% 
  filter(herd_type_ml_description  %in% c("Dairy", "Beef", "Fattener", "Mixed", "Store")) %>% 
  droplevels() %>% 
  split(list(.$bd_end_yr)) %>%
  map(~tabyl(.x, herd_type_ml_description)) %>% 
  map(., ~mutate(.x, percent = round(percent, 2)) %>% select(-n)) %>% 
  imap(~mutate(.x, year = .y)) %>% 
  bind_rows() %>%
  mutate(year = as.numeric(year))
#unique(prop_herd_type_bds$herd_type_ml_description)
prop_herd_type_bds_bar_plot <- ggplot(prop_herd_type_bds, aes(x = year, y = percent, fill = herd_type_ml_description)) +
  ...
  labs(x = "Year", 
       ...
       #fill = "AHCS herd type",
       title =
         paste0("Proportion of all BDs by herd-type"),
       caption = "Year corresponds to BD end year") +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +
  scale_x_continuous(breaks = years_for_study) +
  scale_y_continuous(labels = scales::label_percent(scale = 100)) 
prop_herd_type_bds_bar_plot
```
...
#| fig.width: 12
#| fig.height: 14
#| fig.cap: "Boxplots of the number of bTB cases in BDs that ended in each year by AHCS herd-types."
ggplot_boxplot_half_plot(bd_df,  
                         bd_end_yr_f, all_cases, bd_end_yr_f, y_zoom = 20, facet_var = herd_type, no_of_columns = 1) +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +
  labs(x = "BD end year",
       y = "All cases")
...
#| fig.cap: "Boxplots of the number of bTB cases in BDs that ended in each year by AHCS herd-types."
ggplot_boxplot_half_plot(bd_df,  
                         bd_end_yr_f, all_cases, bd_end_yr_f, 
                         #y_zoom = 20, 
...
#| fig.width: 12
#| fig.height: 14
#| fig.cap: "Boxplots of the number of bTB cases in BDs that ended in each year by Brock et al. herd-types (only including the 4 most common herd-types)."
ggplot_boxplot_half_plot(bd_df %>% 
                           filter(herd_type_ml_description  %in% c("Dairy", "Beef", "Fattener", "Mixed")),  
                         bd_end_yr_f, all_cases, bd_end_yr_f, y_zoom = 20, facet_var = herd_type_ml_description, no_of_columns = 1) +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1))  +
  labs(x = "BD end year",
       y = "All cases")
...
### Brock et al. dairy sub-class herd-types by BD size
...
#| fig.width: 12
#| fig.height: 14
#| fig.cap: "Boxplots of the number of bTB cases in BDs that ended in each year by Brock et al. sub-classes by dairy herd-types only."
ggplot_boxplot_half_plot(bd_df %>% 
                           filter(str_detect(herd_type_ml_sub_class_description, "Dairy")),
                         bd_end_yr_f, all_cases, bd_end_yr_f, y_zoom = 20, facet_var = herd_type_ml_sub_class_description, no_of_columns = 1) +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1))  +
  labs(x = "BD end year",
       y = "All cases")
...
all_case_5_plot <- ggplot(all_case_5_df, aes(x = year, y = percent, fill = all_cases_cut_5)) +
  ...
  labs(x = "Year", 
       ...
       caption = "Year corresponds to year the BD ended") +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +
  scale_x_continuous(breaks = years_for_study)
...
all_case_5_df <- bd_df %>% 
  filter(!is.na(herd_type)) %>% 
  split(list(.$bd_end_yr, .$herd_type)) %>%
  map(~tabyl(.x, all_cases_cut_5)) %>% 
  map(., ~mutate(.x, percent = round(percent, 2)) %>% select(-n)) %>% 
  imap(~mutate(.x, year = .y)) %>% 
  bind_rows() %>% 
...
all_case_5_plot <- ggplot(all_case_5_df, aes(x = year, y = percent, fill = all_cases_cut_5)) +
  geom_bar(position = position_fill(reverse = TRUE), stat = "identity", color = 'black', width = 0.9) +
...
all_case_5_df_brock <- bd_df %>% 
  ...
  split(list(.$bd_end_yr, .$herd_type_ml_description)) %>%
...
#| fig.width: 12
#| fig.height: 14
#| fig.cap: "Boxplots of the number of BD durations (in years) by AHCS herd-types (zoomed into 1 year)."
ggplot_boxplot_half_plot(bd_df,  
                        bd_end_yr_f, bd_duration_yrs, bd_end_yr_f, y_zoom = 1, facet_var = herd_type, no_of_columns = 1) +
   theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +
  labs(x = "BD end year",
       y = "BD duration (years)")
...
bd_df %>% 
  ...
  split(list(.$bd_end_yr_f, .$herd_type_ml_description)) %>%
...
## Count of no. BDs by detection method

### SICTT initiated BDs

```{r}
#| label: fig-countofBDsictt
#| include: TRUE
#| echo: FALSE
#| eval: TRUE
#| warning: FALSE
...
#| fig.width: 12
#| fig.height: 8 
#| fig.cap: "The count of the number of BDs initiated by SICTT, by herd-type."
bd_df %>% 
  filter(bd_initiated == "SICTT") %>% 
  filter(herd_type_ml_description  %in% c("Dairy", "Beef", "Fattener", "Mixed", "Store", "Trader")) %>% 
  group_by(bd_end_yr_f, herd_type_ml_description) %>% 
  summarise(no_of_bds_started = n()) %>% 
  ungroup() %>% 
  ggplot(aes(x = bd_end_yr_f, y = no_of_bds_started, fill = herd_type_ml_description)) +
  # Implement a grouped bar chart
  geom_col(position = position_dodge2(preserve = "single", padding = 0.1)) +
  labs(x = "Year",
       y = "No. of BDs initiated",
       title = paste0("No. of BDs initiated by SICTT, by herd-type"),
       caption = paste0()) +
  theme_jamie +
  theme_45_x +
  scale_fill_manual(name = "Herd-type", values = brock_cols) +
  # scale_y_continuous(labels = scales::label_percent(scale = 100)) +
  # scale_x_type +
...
#| fig.width: 12
#| fig.height: 8 
#| fig.cap: "The count of the number of BDs initiated by a slaughter-house case, by herd-type."
bd_df %>% 
  filter(bd_initiated == "Slaughter (non permit animal)") %>% 
  filter(herd_type_ml_description  %in% c("Dairy", "Beef", "Fattener", "Mixed", "Store", "Trader")) %>% 
  group_by(bd_end_yr_f, herd_type_ml_description) %>% 
  ...
  labs(x = "Year",
       y = "No. of BDs initiated",
       title = paste0("No. of BDs initiated by slaughter-house case, by herd-type"),
       caption = paste0()) +
  theme_jamie +
  theme_45_x +
  scale_fill_manual(name = "Herd-type", values = brock_cols) +
...
# *************************************************************************
# this is temp fix until i rerun main code with this in it
bd_df <- bd_df %>% 
  mutate(bd_initiated = ifelse(is.na(bd_initiated), "IFN-γ", bd_initiated))
# *************************************************************************
...
no_of_bds_ended_in_year <- bd_df %>% 
  tabyl(bd_end_yr) %>% 
  mutate(bd_end_in_year = paste0(n, " (", round(percent, 3)*100, ")")) %>% 
  select(-n, -percent) %>% 
...
final_table_bd_stats <- force_bind_year(  no_of_unique_herds_with_case,
                                          #bd data
                                          no_of_bds_ended_in_year,
                                          bd_median_t,
                                          bd_range_t,
                                          bd_by_cat, 
                                          bd_initiated_year, force_to_character = TRUE, drop_years = "2026") %>% 
...
bd_duration_tab <- bd_df %>% 
  ...
  select(-sum) %>% 
  mutate(bd_end_yr = as.character(bd_end_yr))
...
ft <- flextable(bd_duration_tab) %>% 
  ...
  set_caption(paste0("Table 1: Numbers presented as n (%) where appropriate")) %>% 
  #bold(i = 1) %>%  # bold header
  add_footer_row(values = "Year represents year the BD ended in (it could overlap multiple years)",
