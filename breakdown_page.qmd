...
# by herd type
bd_df_per_month_herd_type <- bd_df %>% 
  mutate(year_month = floor_date(bd_start , "month")) %>% 
  #select(herd_no, bd_start, year_month) 
...
bd_df %>% 
  ...
  group_by(bd_start_yr_f, herd_type_ml_description) %>% 
  summarise(no_of_bds_started = n()) %>% 
  ungroup() %>% 
  ggplot(aes(x = bd_start_yr_f, y = no_of_bds_started, fill = herd_type_ml_description)) +
  # Implement a grouped bar chart
...
final_table_bd_stats <- force_bind_year(  no_of_unique_herds_with_case,
                                          ...
                                          bd_range_t,
                                          bd_by_cat, 
                                          bd_initiated_year, force_to_character = TRUE, drop_years = "2025") %>% 
