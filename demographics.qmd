...
Data is available for 2005-current but we found many instances in the early years where a herd was in a BD (`trading_status == withdrawn`) but no bTB cases were reported (the BD may have begun before our earliest date) and so to make fair comparisons between years we have only included data from 2008 onwards.
...
The following is the revised decision tree for Brock et al. Also included are the original decision tree from the paper and also an intermediary decision tree that was also explored. 
...
```{r}
#| label: fig-brockdecisiontreeintermediate
#| include: TRUE
#| echo: FALSE
#| eval: TRUE
#| warning: FALSE
...
#| fig.width: 12
#| fig.height: 8 
#| fig.cap: "Intermediate decision tree/algorithm of Brock et al. used for classifying herds"

brock_decision_tree_intermediate
```
