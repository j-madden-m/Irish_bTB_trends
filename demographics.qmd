...
# *************************************************************************
# read in Jonas Brock updated decision tree ----
# *************************************************************************
# density = 300 sets the resolution to 300 DPI for clearer images.
library(magick)
brock_decision_tree <- image_read_pdf("D:/Files/bTB_breakdown_trends_paper/figures/brock_decision_tree/decision_tree_update.pdf",
                                  density = 300) %>% 
  image_ggplot(interpolate = TRUE)

# intermediate one
brock_decision_tree_intermediate <- image_read_pdf("D:/Files/bTB_breakdown_trends_paper/figures/brock_decision_tree/decision_tree_update_intermediate.pdf",
                                  density = 300) %>% 
  image_ggplot(interpolate = TRUE)

# original from paper
brock_decision_tree_original <- image_read("D:/Files/bTB_breakdown_trends_paper/figures/brock_decision_tree/orginal_paper_decision_tree.webp",
                                                   density = 300) %>% 
  image_ggplot(interpolate = TRUE)
brock_decision_tree_original
...
-   [Brock et al. (2021)](https://www.nature.com/articles/s41598-021-82373-3){target="_blank"} created two model based herd categorisation. [Brock et al. (2022)](https://irishvetjournal.biomedcentral.com/articles/10.1186/s13620-022-00212-x){target="_blank"} used these to explore enterprises from 2015 to 2019, here we expand on this work and additionally explore more years of data. The original categorisation included `Seasonal` herd-types but these are now no longer included. They have moved into `Traders` or `Fatteners` respectively, based on their profiles. The algorithm used in the original study has since been slightly modified (see updated decision tree below, taken from [Brock et al. (2022)](https://irishvetjournal.biomedcentral.com/articles/10.1186/s13620-022-00212-x){target="_blank"}). The Brock classifications for this study used this revised algorithm.
...
The following is the revised decision tree for Brock et al. Also included is the original decision tree from the paper.

::: panel-tabset
## Revised (most recent)

```{r}
#| label: fig-brockdecisiontreerevised
#| include: TRUE
#| echo: FALSE
#| eval: TRUE
#| warning: FALSE
...
#| fig.width: 12
#| fig.height: 10
#| fig.cap: "Revised decision tree/algorithm of Brock et al. used for classifying herds"
...
```{r}
#| label: fig-brockdecisiontreeoriginal
#| include: TRUE
#| echo: FALSE
#| eval: TRUE
#| warning: FALSE
...
#| fig.width: 12
#| fig.height: 10
#| fig.cap: "Original decision tree/algorithm of Brock et al. used for classifying herds"

brock_decision_tree_original
```
:::

**Revised decision tree description, taken from [Fig 1](https://irishvetjournal.biomedcentral.com/articles/10.1186/s13620-022-00212-x){target="_blank"}**:

> A decision tree for the classification of the Irish cattle sector adapted from \[11\]. Histograms represent herds remaining at the respective node (decision variable). Numbers and dashed line demarcate thresholds for class assignment. Main herd types: dairy (D), beef (B), mixed (M), store/rearing (S/R), fattening (F), and trading (T) herds. Dairy sub-types: dairy (D), dairy no rearing—contract (DnR-C), dairy no rearing—no contract (DnR-nC), dairy rearing male calves (DRm). Beef sub-types: beef pedigree (BP), beef suckling to weanlings (BSW), beef suckling to youngstock (BSY), beef suckling to youngstock—no rearing (BSY-nR), beef suckling to beef (BSB). Store/rearing sub-types: store dairy males (Sdm), store beef males (Sbm), Store beef females (Sbf), store beef mixed (Sbmx), rearing dairy females (Rdf). In each of the years between 2015 and 2019, an average of 95% of Irish herds were classified by the left tree, 5% by the right.

**Original decision tree description, taken from [Fig 5](https://www.nature.com/articles/s41598-021-82373-3){target="_blank"}**:
...
-   (generally just suckler from AHCS)
-   Residency in the herd is longer than 30 days for at least 50% of the herd
-   Proportion of animals in a herd that have ever calved by September, relative to herd size is greater than 25%.
...
-   Animals are kept in the herd for only part of the year. (As of 2024, "seasonal" is no longer a category - they have been moved into "trader" and "fattener" categories respectively). The ‘seasonal’ herds are those in the top right of the diagram – basically any herd that had animals at at least one of but fewer than three of 1st January, May, September.

**Unknown** - Herds that didnt have animals in them at any of the three time points (1st January, May, September)
...
# *************************************************************************
# this for the paper
herd_type_jonas_percent_for_paper <- herd_type_master_tb %>% 
  filter(!is.na(herd_type_ml_description)) %>% 
  split(.$skin_fixed_test_date_year) %>%
  ...
  bind_rows() %>% 
  mutate(percent_label_no_trader_or_unknown = percent*100,
         percent_label_no_trader_or_unknown = ifelse(herd_type_ml_description %in% c("Trader", "Unknown"),
                                                     "", 
                                                     percent_label_no_trader_or_unknown))
# save data
saveRDS(herd_type_jonas_percent_for_paper, paste0("data/edited_output/for_paper/herd_type_jonas_percent_for_paper.rds"))
# *************************************************************************
...
jonas_herd_type_short_bar <- ggplot(herd_type_jonas_percent_for_paper, aes(x = year, y = percent, fill = herd_type_ml_description)) +
  geom_bar(position = "fill", stat = "identity", color = 'black', width = 0.9) +
  geom_text(
    #data = percentage_year %>% 
     #         filter(!herd_type_ml_description  %in% c("Trader", "Unknown")),
            #aes(label = scales::percent(percent, accuracy = 0.1)), 
    # lets remove % from label
    aes(label = percent_label_no_trader_or_unknown),
  ...
  labs(x = "Year", y = "Proportion of herd-type", fill = "Brock herd type",
       title =
         paste0("Proportion of all herds by herd-type"# and year"
         #(", min(percentage_year$year), 
          #      " - ", max(percentage_year$year), ")"
         ),
       caption = paste0("Numbers on the plot represent %'s",
                        "\n Trader and unknown %'s removed for clarity")
       #caption = "Newly established herds: herds that did not have a SICTT before 2015 (but had at least one after 2015)"
       ) #+
  #guides(fill = guide_legend(nrow = 1))
jonas_herd_type_short_bar
```
...
## Bovine population (usually a year lag)
...
# for paper:
saveRDS(percentage_year_no_na_may, paste0("data/edited_output/for_paper/percentage_year_no_na_may.rds"))
...
### Proportion of animals by herd-type

$~$ $~$

```{r}
#| label: fig-herdpopmayproportionbar
#| include: TRUE
#| echo: FALSE
#| eval: TRUE
#| warning: FALSE
...
#| fig.width: 12
#| fig.height: 10 
#| fig.cap: "Proportion of all animals on 1st May by the herd-type they are residing in."

percentage_year_no_na_may <- percentage_year_no_na_may %>% 
  mutate(percent = round(percent, 0)) %>% 
  mutate(percent_label_no_trader_or_unknown = percent,
         percent_label_no_trader_or_unknown = ifelse(herd_type_ml_description %in% c("Trader", "Unknown"),
                                                     "", 
                                                     percent_label_no_trader_or_unknown))


jonas_no_of_animals_herd_type_short_bar <- ggplot(percentage_year_no_na_may, aes(x = as.character(skin_fixed_test_date_year), y = percent, fill = herd_type_ml_description)) +
  geom_bar(position = "fill", stat = "identity", color = 'black', width = 0.9) +
  geom_text(#data = percentage_year_no_na %>% 
              #filter(!herd_type_ml_description  %in% c("Trader", "Unknown")), aes(label = paste0(round(percent, 1), "%")), #scales::percent(percent, accuracy = 0.1)), 
            aes(label = percent_label_no_trader_or_unknown),
              position = position_fill(vjust = 0.5), 
            size = 4) +
  scale_fill_manual(name = "Brock et al. herd type", values = brock_cols) +
  ...
  background_grid() +
  #scale_y_continuous(labels = scales::percent) +
  labs(x = "Year", y = "Proportion of animals by herd-type", fill = "Brock herd type",
       title =
         paste0("Proportion of all animals on 1st May by herd-type"
                #(", min(percentage_year_no_na$skin_fixed_test_date_year), 
                #" - ", max(percentage_year_no_na$skin_fixed_test_date_year), ")"
                ),
       caption = paste0("Numbers on the plot represent %'s",
       "\n Trader and unknown %'s removed for clarity")
       #Newly established herds: herds that did not have a SICTT before 2015 (but had at least one after 2015)"
  )
jonas_no_of_animals_herd_type_short_bar
```
...
# for paper:
saveRDS(herd_size_median, paste0("data/edited_output/for_paper/herd_size_median.rds"))
...
final_table_herd_demographics <- force_bind_year(  unique_herds_each_year_t,
                                 ...
                                 force_to_character = TRUE, drop_years = "2026") %>% 
