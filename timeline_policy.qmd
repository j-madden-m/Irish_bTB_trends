# Irish bTB eradication programme timeline {#sec-timeline}
...
#| label: setup
#| include: FALSE
#| eval: FALSE # true or false depending if interactive or not
...
herd_level_prevalance_data <- read_csv("data/edited_output/prev_df_05_to_current.csv")
case_count_year <- read_csv("data/edited_output/case_count_year_05_to_current.csv") %>% 
...
timelines <- read_csv("D:/Files/Miriam/policy_timeline/data/timeline_dates.csv") %>% 
  mutate(end_date = ifelse(current_or_once_off == "current", format(Sys.Date(), format = "%d-%m-%Y"), end_date),
         ...
         end_date = if_else(is.na(end_date) & current_or_once_off == "current", as.Date("28-02-2026"), end_date),
...
policy_fixed_period_after_06 <- policy_fixed_period %>% 
  ...
  filter(start_date <= dmy("01-02-2026")) %>% 
  # split string so "\n" is inserted after every second word
  mutate(event_new = map_chr(event, ~ insert_newlines(.x, 2))) %>% 
  #rowwise() %>% 
  #mutate(random_height = sample(1:10, 1)) 
  ...
  mutate(prevalence = seq(0.07, diff_in_prevalence*8, length.out = nrow(.))) %>% 
...
jj <- ggplot(herd_level_prevalance_data, aes(x = start_date, y = prevalence)) +
  ...
  scale_y_continuous(
    ...
    sec.axis = sec_axis(~ . * scale_factor, 
                        name = NULL, #bTB cases
                        breaks = seq(from = 15000, to = 45000, 5000)
                        #, 0.04, 0.05, 0.06),
    )  # Right y-axis label
  ) +
  ...
  geom_text(data = policy_fixed_period_after_06, aes(x = midpoint_date, y = prevalence + 0.0022, label = event), color = "white", fontface = "bold") +
  theme_45_x +
  ...
  labs(x = "Year",
       ...
       title = paste0("Timeline of major trials, diagnostic and policy changes during the Irish bTB eradication (2005 - 2025)"),
       subtitle = paste0(
...
The following image has been extracted from More SJ & Good M [2006](https://www.sciencedirect.com/science/article/pii/S0378113505003986?via%3Dihub){target="_blank"} with permission.
...
## WOAH (World Organisation for Animal Health, founded as OIE), Manual of Diagnostic Tests and Vaccines for Terrestrial Animals
...
## List of EU-Audit reports (Sante F reports) for Ireland 


- Health and Food Audits and Analysis, Sante F reports database <https://ec.europa.eu/food/audits-analysis/audit-report/IE>
...
- Robert O'Connor, 1986. A Study of the Bovine Tuberculosis Eradication Scheme <https://www.esri.ie/publications/a-study-of-the-bovine-tuberculosis-eradication-scheme>
...
- Maher, P., Good, M. & More, S. Trends in cow numbers and culling rate in the Irish cattle population, 2003 to 2006. Ir Vet J 61, 455 (2008). <https://doi.org/10.1186/2046-0481-61-7-455>
